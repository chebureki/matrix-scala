...
def determine_allof_inner_types(schemas: list[Schema | Reference[Schema]], base_name: str,
                                path: Optional[NamespacePath]) -> list[ArgumentType]:
...
def transform_operation(http_method: str, endpoint: str, sop: swagger.Operation) -> codegen.Operation:
    ...
    (path_args, query_args, header_args) = (
    extract_and_transform_params_from("path"), extract_and_transform_params_from("query"),
    extract_and_transform_params_from("header")) if sop.parameters else ([], [], [])
...
def extract_schema_inner_models(schema: Schema) -> list[Model]:
    ...
    prop_tuples = [] if schema.properties is None else list(filter(lambda t: isinstance(t[1],Schema), schema.properties.items()))
    schema_props = list(filter(lambda pt: pt[1].properties is not None,prop_tuples))
    # items type is: Items|Reference, we don't care about references
    array_props = list(filter(lambda pt: ((pt[1].items is not None) and isinstance(pt[1].items, Items)), prop_tuples))

    allOf_props = list(filter(lambda pt: pt[1].allOf is not None,prop_tuples))

    def schema2model(field_name: str, schema: Schema) -> Model:
        model_name = attempt_model_name_from_schema(schema) or to_model_name(field_name)
        return transform_schema(schema, model_name)

    def inner_most(items: Items) -> Items:
        # quite the mouthful :P
        return inner_most(items.items) if items.items else items

    schema_models: list[Model] = [schema2model(*t) for t in schema_props]
    array_models: list[Model] = [schema2model(*t) for t in [(t[0],inner_most(t[1].items)) for t in array_props]]
    allOf_models: list[Model] = list()

    return schema_models+array_models+allOf_models


# parameter and schema are decoupled.
# shame on you swagger v2
def determine_schema_type(schema: Schema | Reference, possible_model_name: str,
                          path: Optional[NamespacePath]) -> ArgumentType:
    ...
    if isinstance(schema.type, list):
        ...
        assert False, f"failed to determine type for {schema.type}"
    ...
    assert False, "failed to determine schema type"
...
def extract_schema_fields(schema: Schema, namespace: NamespacePath) -> list[Argument]:
    ...
    for field_name, field_schema in schema.properties.items():
        ...
        field_type = determine_reference_type(field_schema) if isinstance(field_schema, Reference) else (
            determine_schema_type(field_schema,
                                  attempt_model_name_from_schema(field_schema) or to_model_name(field_name), namespace))
        fields.append(
            Argument(
                name=field_name,
...
def extract_operation_model(op: swagger.Operation) -> Model:
    ...
    def get_body_model() -> Optional[Model]:
        ...
        if body_arg and body_arg.schema:
            name = attempt_model_name_from_schema(body_arg.schema) or "Body"  # not too pretty that it's hardcoded
